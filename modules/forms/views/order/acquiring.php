...
$this->title = "Оформление заявки на эквайринг";
...
<div class="wrapper-form shadow-sm rounded mb-4">
    ...
    <?= $form->field($model, 'priceProduct', ['options' => ['class' => 'form-floating mb-3']])->textInput(); ?>
...
$this->registerJs(
    "
                $('#acquiringform-productid').on('change', function(){
                    if($(this).val() > 0)
                    {
                        setProductPrice($(this).val(), '#acquiringform-priceproduct');
                    }
                });
            "
);
